...
# Filtering out prices that are negative or zero
...
model_auto_ar = auto_arima(train, trace=True, error_action='ignore', suppress_warnings=True)
model_auto_ar.fit(train_data)
forecast_auto_ar = model_auto_ar.predict(n_periods=len(test))
forecast_auto_ar = pd.DataFrame(forecast_auto_ar,index = test.index,columns=['Prediction'])
...
plt.figure(figsize=(10, 5))
plt.plot(df[date_column_name], df[value_column_name], label='Actual Data')
future_dates = pd.date_range(start=df[date_column_name].iloc[-1], periods=n_periods + 1, freq='H')[1:]
plt.plot(future_dates, forecast_values, label='Forecasted Values', color='red')
