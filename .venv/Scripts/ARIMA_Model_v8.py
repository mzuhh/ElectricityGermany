...
# Ensure the date column is parsed correctly with utc=True
df[date_column_name] = pd.to_datetime(df[date_column_name], format="%Y-%m-%dT%H:%M%z", utc=True, errors="coerce")
df = df[df[value_column_name] > 0]

# Sort the DataFrame by the date column to ensure monotonic DatetimeIndex
...
df_daily_training = df.loc[start_date_training:end_date_training]
df_daily_test = df.loc[start_date_testing:]

# Plot the training and test sets
plt.figure(figsize=(18, 6))
plt.title(plot_main_title, fontsize=14)
df_daily_training[value_column_name].plot(label="training set " + start_date_training + " - " + end_date_training, fontsize=14)
df_daily_test[value_column_name].plot(label="test set " + start_date_testing, fontsize=14)
plt.legend()
plt.show()

# Box-Cox transformation
boxcox_transformed_data, boxcox_lambda = boxcox(df_daily_training[value_column_name] + 10)
boxcox_transformed_data = pd.Series(boxcox_transformed_data, index=df_daily_training.index)

fig, ax = plt.subplots(2, 1, figsize=(16, 8))
df_daily_training[value_column_name].plot(ax=ax[0], color="black", fontsize=14)
...
kpss_test(df_daily_training[value_column_name])

df_daily_training_diff1 = df_daily_training[value_column_name].diff()
...
# Fit ARIMA model
model = ARIMA(df_daily_training[value_column_name], order=(fst, snd, trd))
fitted = model.fit()
print(fitted.summary())

# Auto ARIMA with increased maxiter
#auto_model = auto_arima(df_daily_training[value_column_name], maxiter=100, seasonal=False, stepwise=True, suppress_warnings=True)
...
ax[0].grid()
ax[0].set_title(f"Residuals of ARIMA({fst}, {snd}, {trd})")
plot_acf(residuals, ax=ax[1])
ax[1].set_title("ACF of Residuals")
plt.show()

# Ljung-Box test
Btest = acorr_ljungbox(fitted.resid, lags=[10], return_df=True, model_df=5)
print(Btest)
...
df_daily_training[value_column_name].plot(color="black", label="training set" + start_date_training + " " + end_date_training)
plt.plot(fitted.fittedvalues, color="blue", label="fitted values")
plt.legend()
plt.show()

# Perform rolling forecast
history = list(df_daily_training[value_column_name])
predictions = []
conf_ints_95 = []
conf_ints_80 = []

for t in range(len(df_daily_test)):
    model = ARIMA(history, order=(fst, snd, trd))
    model_fit = model.fit()
    ...
    conf_ints_95.append([conf_int_95[0, 0], conf_int_95[0, 1]])
    conf_ints_80.append([conf_int_80[0, 0], conf_int_80[0, 1]])
    history.append(df_daily_test[value_column_name].iloc[t])
...
plt.figure(figsize=(13, 4))
plt.title(plot_main_title)
df_daily_training[value_column_name][start_date_plot:].plot(color="black", label="training set" + start_date_training + " " + end_date_training)
df_daily_test[value_column_name].plot(color="red", label="test set " + start_date_testing)
...
true_values = df_daily_test[value_column_name].values
